...
class ScreenCompiler {
    ...
    // never re-shows the form. This exact binding shape (one "view" binding,
    // no intermediate rename) is confirmed both by wiki.bedrock.dev's
    // buttons-and-toggles page AND by bedrock-core/ui's own shownWhileOn()
    // (used for its <Disclosure> component) - a genuinely proven, working
    // pattern, not a guess. `toggleKey` is the toggle's own control key
    // (this compiler builds toggles by hand - see tabsEl() - so there's no
    // separate $toggle_view_binding_name indirection to worry about; the
    // key IS the addressable name, and bedrock-core's own docs confirm the
    // lookup is screen-wide, not limited to siblings).
    visibleFromToggle(control, toggleKey, invert, defaultOn, place) {
        return {
            type: "panel", ...place,
            visible: invert ? "!#visible" : "#visible",
            property_bag: { "#visible": invert ? !defaultOn : defaultOn },
            bindings: [
                { binding_type: "view", source_control_name: toggleKey, source_property_name: "#toggle_state", target_property_name: "#visible", binding_condition: "always" },
            ],
    ...
    // and cross-checking bedrock-core/ui's own compiler source
    // (packages/ui-compiler/src/faces/utils/swap.ts) explains exactly why a
    // hand-built toggle needs ALL of the following, verbatim, or it "draws
    // but never takes a press" (their own comment):
    ...
    //     below - without these it never takes a press at all, confirmed as
    //     the actual root cause of "pressing the buttons does nothing".
    // Every property below is copied from that proven, shipped source, not
    ...
    tabsEl(node, loops) {
        ...
        tabNodes.forEach((t, i) => {
            ...
            const look = (active) => ({
                ...
                controls: [
                    ...
                    { txt: { type: "label", size: ["100%", "100%"], layer: 2, text, localize, font_size: "normal", font_scale_factor: 0.85, shadow: false, text_alignment: "center", color: hexColor(barSt[active ? "tab-active-color" : "tab-color"] ?? (active ? "#1c1a26" : "#c8ccec"), e) } },
                ],
            });
        ...
        tabNodes.forEach((t, i) => {
            ...
            flat.push({ [this.name(`tabbody_${t.attrs.id}`)]: this.visibleFromToggle(body, toggleKeys[i], false, t.attrs.id === defaultId, bodyPlace) });
        });
