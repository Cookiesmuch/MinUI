...
class ScreenCompiler {
    ...
    // transition. `viewName` is a toggle's own $toggle_view_binding_name
    // (NOT its control key - see tabsEl()'s own comment on why that
    // distinction matters), addressed the exact way wiki.bedrock.dev's
    // buttons-and-toggles page documents: one "view" binding, no
    // intermediate rename step.
    visibleFromToggle(control, viewName, invert, defaultOn, place) {
        return {
            type: "panel", ...place,
            visible: invert ? "!#visible" : "#visible",
            property_bag: { "#visible": invert ? !defaultOn : defaultOn },
            bindings: [
                { binding_type: "view", source_control_name: viewName, source_property_name: "#toggle_state", target_property_name: "#visible", binding_condition: "always" },
            ],
    ...
    // mechanism). Compiles to one vanilla toggle per tab, extending
    // common_toggles.light_text_toggle via the same "@template" inheritance
    // this compiler already uses for <scroll> - never a hand-built
    // type:"toggle" object (an earlier attempt did that and got the
    // state-slot names wrong; a toggle uses checked_control/
    // unchecked_control, not a button's default_control/hover_control).
    // Each tab body's visibility binds straight to its own toggle's
    // $toggle_view_binding_name (visibleFromToggle above) - never touches
    // the server, never re-shows the form.
    tabsEl(node, loops) {
        const e = m => this.err(node, m);
        ...
        // The toggle's OWN control key (just needs to be a unique JSON key)
        // and its $toggle_view_binding_name (the string every visibility
        // binding elsewhere actually addresses it by - a real, separate
        // property, confirmed against wiki.bedrock.dev's own worked
        // example) are deliberately two different names, so this never
        // regresses to addressing-by-control-key again.
        const toggleKeys = tabNodes.map(t => this.name(`tabtoggle_${t.attrs.id}`));
        const viewNames = tabNodes.map(t => this.name(`tabview_${t.attrs.id}`));
        ...
        tabNodes.forEach((t, i) => {
            ...
            const color = k => hexColor(barSt[k] ?? (k.includes("checked") ? "#1c1a26" : "#c8ccec"), e);
            flat.push({
                [`${toggleKeys[i]}@common_toggles.light_text_toggle`]: {
                    ...place,
                    $button_text: text,
                    localize,
                    $toggle_name: groupName,
                    $radio_toggle_group: true,
                    $toggle_view_binding_name: viewNames[i],
                    $toggle_default_state: isDefault,
                    $default_texture: barSt["background"] ?? "textures/ui/button_borderless_light",
                    $hover_texture: barSt["hover-background"] ?? "textures/ui/button_borderless_lighthover",
                    $pressed_texture: barSt["pressed-background"] ?? "textures/ui/button_borderless_lightpressed",
                    $pressed_no_hover_texture: barSt["pressed-background"] ?? "textures/ui/button_borderless_lightpressed",
                    $default_text_color: color("tab-color"),
                    $hover_text_color: color("tab-color"),
                    $default_checked_text_color: color("tab-active-color"),
                    $hover_checked_text_color: color("tab-active-color"),
                },
            });
            if (barSt["tab-active-background"]) {
                flat.push({ [this.name("tabbgA")]: this.visibleFromToggle({ type: "image", size: [tabW, tabH], texture: barSt["tab-active-background"] }, viewNames[i], false, isDefault, { ...place, layer: 1 }) });
            }
        });
        ...
        tabNodes.forEach((t, i) => {
            ...
            flat.push({ [this.name(`tabbody_${t.attrs.id}`)]: this.visibleFromToggle(body, viewNames[i], false, t.attrs.id === defaultId, bodyPlace) });
        });
